...
showMerch = (data) => {
  ...
  starbucksHTML += `<h3>${data.menus[0].children[2].name}</h3>`;
  // each sub-category has a list of sub-items
  for (const subItem of data.menus[0].children[2].children) {
    starbucksHTML += ` <h4 class = "${subItem.name} ${data.menus[0].children[2].name}">${subItem.name}</h4> <section class="cards ${subItem.name} ${data.menus[0].children[2].name}">`;
  ...
  // Index drinks then index cold drinks
  starbucksHTML += `<h3>${data.menus[0].children[6].name}</h3>`;
  // each sub-category has a list of sub-items
  for (const subItem of data.menus[0].children[6].children) {
    starbucksHTML += ` <h4 class = "${subItem.name} ${data.menus[0].children[6].name}">${subItem.name}</h4> <section class="cards ${subItem.name} ${data.menus[0].children[6].name}">`;
    // each sub-item has variations
    for (const product of subItem.products) {
      // all of the product listings are inside of the sub-item's products list:
      starbucksHTML += makeCard(product);
...
filter = (ev) => {
  ...
  if (ev.target.value.includes("Water") && !ev.target.value.includes("Sparkling")) {
    if (checked) {
